...
# Generate variable importance summaries ----------------------------------

lowo_rfcde_var_imp_loss <-
  map_dfr(1:17,
          function(week_i) {

            variable_importance(lowo_rfcde_models[[week_i]],
                                type = "loss") %>%
              as_tibble() %>%
              mutate(variable_name =
                       names(variable_importance(lowo_rfcde_models[[week_i]],
                                                 type = "loss")),
                     week_id = week_i)

          })

write_rds(lowo_rfcde_var_imp_loss,
          "data/model_output/lowo_rfcde_var_imp_loss.rds")

lowo_rfcde_var_imp_count <-
  map_dfr(1:17,
          function(week_i) {
...
write_rds(lowo_rfcde_var_imp_count,
          "data/model_output/lowo_rfcde_var_imp_count.rds")
          ...
          function(week_i) {
            ...
            map_dfr(1:nrow(week_matrix),
                    function(play_i) {
                      ...
                      obs_max_x <- week_matrix[[play_i, "adj_bc_x"]]
...
write_rds(init_rfcde_pred_yac_data,
          "data/model_output/lowo_rfcde_pred_yac_data_w_padding.rds")
...
write_rds(pred_yac_distr_summary,
          "data/model_output/lowo_pred_yac_distr_summary_w_padding.rds")
                          ...
                          new_rfcde_pred_yac <-
                            map_dfr(1:nrow(new_data_matrix),
                                    function(play_i) {
                                      ...
                                      # What's the distance from the endzone:
                                      obs_max_x <- new_data_matrix[[play_i, "adj_bc_x"]]
...
write_rds(ghost_yac_distr_summary,
          "data/ghosting_output/player_ghost_full_yac_distr_summary_w_padding.rds")
